...
for batch = 1:NumberofBatches
    ...
    for i=1:length(RunIDs)
        ...
        disp("==== Checking dimensions for RunID: " + RunIDs(i) + " ====");
        disp("tout size: " + mat2str(size(simOut(1,1).tout)));
        disp("axmeas size: " + mat2str(size(simOut(1,indexes).axmeas)));
        disp("xmeas size: " + mat2str(size(simOut(1,indexes).xmeas)));
        disp("xmv size: " + mat2str(size(simOut(1,indexes).xmv)));
        res = [simOut(1,indexes).tout,simOut(1,indexes).axmeas, simOut(1,indexes).xmeas, simOut(1,indexes).xmv];
        keep_mask = readtable("keep_mask.csv");    
        keep_mask = table2array(keep_mask);        
        keep_mask = logical(keep_mask(:));          
        res = res(:, keep_mask); 
        res = res(1:((t/Ts_save)+1),:);
        % res(:, res(4, :) == res(5, :)) = [];
